...
@if(Session::has('user_id') && Session::get('role_id') == 1)
    ...
    <div class="main-content">
        ...
        <div class="filter-save-row">
            ...
            <form action="{{ route('superadmin.attendance') }}" method="GET" class="controls">
                ...
                <table class="data-table" id="otDataTable">
                    ...
                    <tbody>
                        ...
                        <tr data-employee-id="{{ $employee->id }}">
                            ...
                            <td><input type="number" step="0.01" class="ot-input ot-hours" data-employee="{{ $employee->id }}" placeholder="0.00" min="0" max="4" value="{{ $att ? $att->overtime_hours : '' }}" readonly style="background:#f3f4f6;" title="Auto-calculated from OT In/Out"></td>
                        </tr>
                        @empty
                        <tr><td colspan="5" class="no-data">No employees found for this date.</td></tr>
                        @endforelse
...
<script>
    ...
    const STORE_URL     = "{{ route('superadmin.attendance.store') }}";
    const STORE_NSD     = "{{ route('superadmin.attendance.storeNsd') }}";
    ...
    function calculateOTHours(employeeId) {
        ...
        const hrs = Math.min((outMins - inMins) / 60, 5).toFixed(2);
        hoursEl.value = hrs;
    ...
    function validateOTTime(input, fieldName) {
        ...
        if (time < '17:00' || time > '21:00') {
            input.classList.add('time-error'); alert(`${fieldName} must be between 5:00 PM (17:00) and 9:00 PM (21:00)`);
            input.value=''; input.classList.remove('time-error'); return false;
        }
        input.classList.remove('time-error'); return true;
    }

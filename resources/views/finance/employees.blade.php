...
@if(Session::has('user_id') && Session::get('role_id') == 4)
    ...
    <div class="main-content">
        ...
        <!-- CONTROLS -->
        <form method="GET" action="{{ route('finance.employees') }}" class="controls-bar" id="filterForm">
    ...
    <div id="employeeAddModal" class="modal-overlay">
        <div class="modal-box" style="max-width:860px;">
            ...
            <h2>➕ Add Employee</h2>
            <hr>
            <form id="addEmployeeForm" action="{{ route('finance.employees.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-section-title">Profile Photo</div>
                <div class="form-row" style="align-items:center;gap:20px;margin-bottom:20px;">
    ...
    <script>
        ...
        async function submitBulkArchive() {
            ...
            for (const id of ids) {
                try {
                    const res = await fetch(`/finance/employees/${id}/archive`, {
                        method: 'POST',
        ...
        function editEmployee(id) {
            fetch(`/finance/employees/${id}/modal`)
                .then(r => r.json())
                .then(d => {
                    ...
                    document.getElementById('employeeEditForm').action = `/finance/employees/${id}`;
        ...
        function openArchiveModal(id, firstName, lastName) {
            ...
            fetch(`/finance/employees/${id}/archive-precheck`, { headers: { 'Accept': 'application/json' } })
                .then(r => r.json())
                .then(data => {
        ...
        function submitArchive() {
            ...
            fetch(`/finance/employees/${savedId}/archive`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' },
        ...
        function fetchEmployees(page = 1) {
            ...
            fetch(`{{ route('finance.employees') }}?${params}`, {

...
{{-- POST store form --}}
<form id="postForm" action="{{ route('finance.holiday.store') }}" method="POST" style="display:none">
    @csrf
...
function saveHoliday() {
    ...
    if (method === 'PUT') {
        ...
        form.action = `/finance/holiday/${id}`;
        form.submit();
    } else {
...
function confirmDelete() {
    ...
    form.action = `/finance/holiday/${pendingDeleteId}`;
    form.submit();
}

...
@if(Session::has('user_id') && Session::get('role_id') == 2)
    ...
    <div class="main-content">
        ...
        <!-- CONTROLS -->
        <form method="GET" action="{{ route('admin.users') }}" class="controls-bar">
        ...
        <table>
            ...
            <tbody id="employeeTableBody">
                @include('admin.partials.user', ['users' => $users])
            </tbody>
        </table>
    ...
    <div id="viewUserModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <table class="details-table">
                ...
                <tr><th>Role</th>      <td id="vRoleId"></td></tr>
    ...
    <div id="editUserModal" class="modal-overlay">
        <div class="modal-box" style="max-width:600px;">
            ...
            <form id="editUserForm" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" id="editUserIdField"        name="edit_user_id"  value="">
                <input type="hidden" id="editUserRemovePhotoFlag" name="remove_photo"  value="0">

                {{-- Photo --}}
                <div class="modal-section-title">Profile Photo</div>
                <div class="form-row" style="align-items:center;gap:20px;margin-bottom:20px;">
                    <div id="editUserAvatarPreview"
                         style="width:90px;height:90px;border-radius:50%;flex-shrink:0;
                                ...
                                box-shadow:0 4px 12px rgba(99,102,241,.3);position:relative;cursor:pointer;"
                         onclick="document.getElementById('editUserPhotoInput').click()">
                        <span id="editUserAvatarInitial" style="pointer-events:none;">👤</span>
                        <img  id="editUserAvatarImg" src="" alt=""
                              style="display:none;width:100%;height:100%;object-fit:cover;position:absolute;inset:0;">
                        <div  id="editUserAvatarOverlay"
                    ...
                    <div>
                        <input type="file" id="editUserPhotoInput" name="photo"
                               accept="image/jpeg,image/png,image/webp" style="display:none;"
                               onchange="previewEditUserPhoto(this)">
                        <button type="button"
                                style="padding:8px 16px;background:#f1f5f9;border:1px solid #cbd5e1;
                                       border-radius:8px;font-size:13px;cursor:pointer;font-weight:600;color:#374151;"
                                onclick="document.getElementById('editUserPhotoInput').click()">📁 Choose Photo</button>
                        <p style="font-size:12px;color:#94a3b8;margin-top:6px;">JPG, PNG or WEBP · Max 2MB</p>
                        <button type="button" id="editUserRemovePhotoBtn" onclick="removeEditUserPhoto()"
                                style="display:none;margin-top:6px;padding:5px 12px;background:#fee2e2;
                                       border:1px solid #fca5a5;border-radius:6px;font-size:12px;
                                       cursor:pointer;color:#dc2626;font-weight:600;">🗑️ Remove Photo</button>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>New Password
                            <span style="color:#9ca3af;font-weight:400;">(leave blank to keep current)</span>
                        </label>
        ...
        <div class="modal-box" style="max-width:420px;">
            ...
            <p style="margin-bottom:16px;color:#374151;font-size:14px;">
                ...
                <span style="color:#dc3545;font-size:13px;">
                    ⚠️ Terminated or Inactive users will not be able to log in.
                </span>
            </p>
            <div class="form-group">
                <label>Status <span style="color:red">*</span></label>
                <select id="archiveStatus"
                        style="width:100%;padding:10px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;">
            ...
            <div id="archiveError"
                 style="display:none;background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;
                        border-radius:6px;padding:8px 12px;font-size:13px;margin-bottom:12px;"></div>
            <div class="form-footer">
                <button type="button" class="btn-cancel" onclick="closeArchiveModal()">✕ Cancel</button>
                <button type="button" id="archiveSubmitBtn" class="btn-save"
                        style="background:#dc3545;" onclick="submitArchive()">🚫 Apply Status</button>
            </div>
        </div>
    </div>
    ...
    <div id="modalOverlay" class="modal-overlay">
        <div class="modal-box" style="max-width:900px;">
            ...
            <hr>
            <form action="{{ route('admin.store') }}" method="POST" id="registerForm"
                ...
                <div class="modal-section-title">Profile Photo</div>
                <div class="form-row" style="align-items:center;gap:20px;margin-bottom:20px;">
                    <div id="addUserAvatarPreview"
                         style="width:90px;height:90px;border-radius:50%;flex-shrink:0;
                                background:linear-gradient(135deg,#6366f1,#4f46e5);
                                display:flex;align-items:center;justify-content:center;
                                font-size:36px;font-weight:700;color:#fff;overflow:hidden;
                                box-shadow:0 4px 12px rgba(99,102,241,.3);position:relative;cursor:pointer;"
                         onclick="document.getElementById('addUserPhotoInput').click()">
                        <span id="addUserAvatarInitial" style="pointer-events:none;">👤</span>
                        <img  id="addUserAvatarImg" src="" alt=""
                              style="display:none;width:100%;height:100%;object-fit:cover;position:absolute;inset:0;">
                        <div  id="addUserAvatarOverlay"
                              style="position:absolute;inset:0;background:rgba(0,0,0,.35);
                                     display:flex;align-items:center;justify-content:center;
                                     border-radius:50%;opacity:0;transition:opacity .2s;">
                            <span style="color:#fff;font-size:20px;">📷</span>
                    ...
                    <div>
                        <input type="file" id="addUserPhotoInput" name="photo"
                               accept="image/jpeg,image/png,image/webp" style="display:none;"
                               onchange="previewUserPhoto(this)">
                        <button type="button"
                                style="padding:8px 16px;background:#f1f5f9;border:1px solid #cbd5e1;
                                       border-radius:8px;font-size:13px;cursor:pointer;font-weight:600;color:#374151;"
                                onclick="document.getElementById('addUserPhotoInput').click()">📁 Choose Photo</button>
                        <p style="font-size:12px;color:#94a3b8;margin-top:6px;">JPG, PNG or WEBP · Max 2MB · Optional</p>
                        <button type="button" id="addUserRemovePhotoBtn" onclick="removeUserPhoto()"
                                style="display:none;margin-top:6px;padding:5px 12px;background:#fee2e2;
                                       border:1px solid #fca5a5;border-radius:6px;font-size:12px;
                                       cursor:pointer;color:#dc2626;font-weight:600;">🗑️ Remove Photo</button>
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group"><label>Last Name <a>*</a></label>
                        <input type="text" name="last_name" value="{{ old('last_name') }}" required></div>
                    <div class="form-group"><label>First Name <a>*</a></label>
                        <input type="text" name="first_name" value="{{ old('first_name') }}" required></div>
                    <div class="form-group"><label>Middle Name</label>
                        <input type="text" name="middle_name" value="{{ old('middle_name') }}"></div>
                    <div class="form-group"><label>Suffix</label>
                        <input type="text" name="suffixes" value="{{ old('suffixes') }}"></div>
                </div>
                <div class="form-row">
                    <div class="form-group"><label>Contact Number <a>*</a></label>
                        <input type="text" name="contact_number" value="{{ old('contact_number') }}" required></div>
                    <div class="form-group"><label>Birthdate <a>*</a></label>
                        <input type="date" name="birthdate" value="{{ old('birthdate') }}" required></div>
                    <div class="form-group"><label>Username <a>*</a></label>
                        <input type="text" name="username" value="{{ old('username') }}" required></div>
                    <div class="form-group"><label>Password <a>*</a></label>
                        <input type="password" name="password" required></div>
                    <div class="form-group"><label>Confirm Password <a>*</a></label>
                ...
                <div class="form-row">
                    <div class="form-group"><label>House Number</label>
                        <input type="text" name="house_number" value="{{ old('house_number') }}"></div>
                    <div class="form-group"><label>Purok <a>*</a></label>
                        <input type="text" name="purok" value="{{ old('purok') }}" required></div>
                    <div class="form-group"><label>Barangay <a>*</a></label>
                        <input type="text" name="barangay" value="{{ old('barangay') }}" required></div>
                    <div class="form-group"><label>City <a>*</a></label>
                        <input type="text" name="city" value="{{ old('city') }}" required></div>
                    <div class="form-group"><label>Province <a>*</a></label>
                        <input type="text" name="province" value="{{ old('province') }}" required></div>
                </div>

                <div class="modal-section-title">Government IDs
                    <span style="font-weight:400;color:#999;">(optional)</span>
                </div>
                <div class="form-row">
                    <div class="form-group"><label>SSS</label>
                        <input type="text" name="sss" value="{{ old('sss') }}"></div>
                    <div class="form-group"><label>Philhealth</label>
                        <input type="text" name="philhealth" value="{{ old('philhealth') }}"></div>
                    <div class="form-group"><label>Pagibig</label>
                        <input type="text" name="pagibig" value="{{ old('pagibig') }}"></div>
                </div>

                <div class="modal-section-title">Emergency Contact</div>
                <div class="form-row">
                    <div class="form-group"><label>Last Name <a>*</a></label>
                        <input type="text" name="last_name_ec" value="{{ old('last_name_ec') }}" required></div>
                    <div class="form-group"><label>First Name <a>*</a></label>
                        <input type="text" name="first_name_ec" value="{{ old('first_name_ec') }}" required></div>
                    <div class="form-group"><label>Middle Name</label>
                        <input type="text" name="middle_name_ec" value="{{ old('middle_name_ec') }}"></div>
                    <div class="form-group"><label>Email</label>
                        <input type="email" name="email_ec" value="{{ old('email_ec') }}"></div>
                </div>
                <div class="form-row">
                    <div class="form-group"><label>Contact Number <a>*</a></label>
                        <input type="text" name="contact_number_ec" value="{{ old('contact_number_ec') }}" required></div>
                    <div class="form-group"><label>House Number</label>
                        <input type="text" name="house_number_ec" value="{{ old('house_number_ec') }}"></div>
                    <div class="form-group"><label>Purok <a>*</a></label>
                        <input type="text" name="purok_ec" value="{{ old('purok_ec') }}" required></div>
                    <div class="form-group"><label>Barangay <a>*</a></label>
                        <input type="text" name="barangay_ec" value="{{ old('barangay_ec') }}" required></div>
                </div>
                <div class="form-row">
                    <div class="form-group"><label>City <a>*</a></label>
                        <input type="text" name="city_ec" value="{{ old('city_ec') }}" required></div>
                    <div class="form-group"><label>Province <a>*</a></label>
                        <input type="text" name="province_ec" value="{{ old('province_ec') }}" required></div>
                    <div class="form-group"><label>Country</label>
                        <input type="text" name="country_ec" value="{{ old('country_ec') }}"></div>
                </div>
    ...
    <script>
        ...
        // ═══════════ EDIT PHOTO ═══════════
        function previewEditUserPhoto(input) {
            if (!input.files || !input.files[0]) return;
            const file = input.files[0];
            if (file.size > 2 * 1024 * 1024) { alert('Image must be under 2MB.'); input.value = ''; return; }
            const reader = new FileReader();
            reader.onload = e => {
                document.getElementById('editUserAvatarImg').src                = e.target.result;
                document.getElementById('editUserAvatarImg').style.display      = 'block';
                document.getElementById('editUserAvatarInitial').style.display  = 'none';
                document.getElementById('editUserRemovePhotoBtn').style.display = 'inline-block';
                document.getElementById('editUserRemovePhotoFlag').value        = '0';
            };
            reader.readAsDataURL(file);
        }
        ...
        function removeEditUserPhoto() {
            ...
            document.getElementById('editUserAvatarImg').src                 = '';
            document.getElementById('editUserAvatarImg').style.display       = 'none';
            document.getElementById('editUserRemovePhotoBtn').style.display  = 'none';
            document.getElementById('editUserRemovePhotoFlag').value         = '1';
            const first = document.getElementById('edit_first_name').value;
            const init  = document.getElementById('editUserAvatarInitial');
            init.textContent   = (first || '?').charAt(0).toUpperCase();
        ...
        function openViewModal(userId) {
            ...
            fetch(`/admin/users/${userId}/status`)
                .then(r => r.json())
                .then(data => {
                    const color = data.status === 'active'     ? '#198754'
                                : data.status === 'terminated' ? '#dc3545' : '#6c757d';
                    ...
                    document.getElementById('vStatus').innerHTML =
                        `<span style="padding:2px 10px;border-radius:12px;font-size:12px;
                                      font-weight:600;color:white;background:${color}">${label}</span>`;
                })
        ...
        function openEditModal(userId) {
            ...
            document.getElementById('editUserIdField').value             = userId;
            document.getElementById('editUserRemovePhotoFlag').value     = '0';
            document.getElementById('editUserPhotoInput').value          = '';

            // Load photo
            const img   = document.getElementById('editUserAvatarImg');
            const init  = document.getElementById('editUserAvatarInitial');
            const rmBtn = document.getElementById('editUserRemovePhotoBtn');

            img.style.display   = 'none';
            init.textContent    = (u.first_name || '?').charAt(0).toUpperCase();
            init.style.display  = 'block';
            rmBtn.style.display = 'none';

            // Fetch photo from employee record
            fetch(`/admin/employees/by-username/${encodeURIComponent(u.username)}`, {
        ...
        function submitArchive() {
            ...
            fetch(`/admin/users/${savedId}/status`, {
                method: 'POST',
                headers: {
        ...
        const editUserAvatar = document.getElementById('editUserAvatarPreview');
        if (editUserAvatar) {
            const euOv = document.getElementById('editUserAvatarOverlay');
            editUserAvatar.addEventListener('mouseenter', () => euOv.style.opacity = '1');
            editUserAvatar.addEventListener('mouseleave', () => euOv.style.opacity = '0');
        }
        ...
        // ═══════════ AUTO-OPEN CORRECT MODAL ON VALIDATION ERROR ═══════════
        // Placed at bottom so all functions are defined first
        @if($errors->any())
            @if(old('edit_user_id'))
                openEditModal({{ old('edit_user_id', 0) }});
            @else
                openModal();
            @endif
        @endif
